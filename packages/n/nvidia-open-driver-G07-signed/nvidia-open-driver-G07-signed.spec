#
# spec file for package nvidia-open-driver-G07-signed
#
...
%if "%{flavor}" == "cuda"
%define check_pkg nvidia-open-driver-G07-signed-cuda-check
%else
%define check_pkg nvidia-open-driver-G07-signed-check
%endif
...
%endif
Name:           nvidia-open-driver-G07-signed%{?with_cuda:-cuda}
%if %{with cuda}
...
%(echo "%triggerin -p /bin/bash -n %%{-n*}-kmp-%1 -- nvidia-common-G07 = %{version}"      >> %_builddir/nvidia-kmp-template)

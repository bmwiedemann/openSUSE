# Ruby 4.0 micro runtime Container Image
...
```Dockerfile
FROM registry.opensuse.org/opensuse/bci/ruby:4.0-micro
...
```ShellSession
$ podman run --rm -v "$PWD":/app:Z -w /app registry.opensuse.org/opensuse/bci/ruby:4.0-micro bundle lock
```

To run a single script inside a container, use the following command:

```ShellSession
$ podman run --rm -v "$PWD":/app:Z -w /app registry.opensuse.org/opensuse/bci/ruby:4.0-micro ruby script.rb
```

# Ruby 4.0 development Container Image
...
```Dockerfile
FROM registry.opensuse.org/opensuse/bci/ruby:4.0-base
...
```ShellSession
$ podman run --rm -v "$PWD":/app:Z -w /app registry.opensuse.org/opensuse/bci/ruby:4.0-base bundle lock
```

To run a single script inside a container, use the following command:

```ShellSession
$ podman run --rm -v "$PWD":/app:Z -w /app registry.opensuse.org/opensuse/bci/ruby:4.0-base ruby script.rb
```

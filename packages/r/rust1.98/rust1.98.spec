#
# spec file for package rust1.98
#
...
%global version_previous 1.97.1

%if 0%{?gcc_version} < 15 && 0%{?suse_version} < 1600
# bsc#1261876 - limit this to SLE 15 and lower, as sle 16 defaults to gcc-13,
# and when you mix it with gcc-15 it confuses things.
#
...
Obsoletes:      %{1}1.96%{?2:-%{2}} \
Obsoletes:      %{1}1.95%{?2:-%{2}} \
Obsoletes:      %{1}1.94%{?2:-%{2}} \
...
%define rust_wasi_targets %{?with_wasi:,wasm32-wasip1}
%define rust_wasm_targets %{?with_wasm:,wasm32-unknown-unknown}

# Base Rust targets for all architectures
%define rust_base_targets %{rust_triple}%{rust_wasm_targets}%{rust_wasi_targets}
...
#
# Clang is now REQUIRED to build wasm/wasi targets.
#

# Depending on our environment, we may need to configure our linker in a different manner.

# === Enable wasip1 on t1 opensuse targets ===
%if 0%{?suse_version} >= 1699
%ifarch x86_64 aarch64
%bcond_without llvmtools
%bcond_without wasi
%bcond_without wasm
%else
%bcond_without llvmtools
...
%if %{with llvmtools}
# There is an annoying issue in the cc crate where it still falls back to
# cc sometimes. This normally wouldn't be an issue but gcc forces some
# linker flags that don't exist in lld. To avoid this we put a cc link
# into our rust_root to clang to try and bypass that.
ln -s /usr/bin/clang %{rust_root}/bin/cc
...
%if 0%{?need_gcc_version} != 0
cat > .env.sh <<EOF
export CC="gcc-%{need_gcc_version}"
export CXX="g++-%{need_gcc_version}"
export RUSTFLAGS="%{rustflags} -Clinker=%{rlinker}"
EOF
%else
cat > .env.sh <<EOF
...
export PATH="%{_builddir}/rustc-%{version}-src/build/%{rust_triple}/llvm/bin:%{rust_root}/bin:${PATH}"
...
%{python} src/bootstrap/configure.py \
  ...
  %{?with_llvmtools: --set llvm.use-linker=lld --set rust.bootstrap-override-lld=true} \
...
rm -rf %{buildroot}%{_docdir}/rust/{README.md,COPYRIGHT*,LICENSE*,licenses}
...
%{python} ./x.py test --target=%{rust_triple} \
    ...
    --exclude tests/run-make/embed-source-dwarf \
    ...
    --exclude tests/run-make/linker-plugin-lto-fat \
    --exclude tests/run-make/remap-path-prefix-std \
    --exclude tests/run-make/pointer-auth-link-with-c \
    --exclude tests/rustdoc-ui \
...
%{rustlibdir}/wasm32-wasip1/lib/*.rmeta

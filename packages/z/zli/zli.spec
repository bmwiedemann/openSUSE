...
# todo dynamically get the tags and the BinaryType argument
go build \
   -mod=vendor \
   -buildmode=pie \
   -tags imagetrust,lint,metrics,mgmt,profile,scrub,search,sync,ui,userprefs,search,containers_image_openpgp \
   -trimpath \
   -ldflags=" \
   -X zotregistry.dev/zot/pkg/api/config.Commit=${COMMIT_HASH} \

...
# Generated by obs-service-go_modules with replace for CVE fixes (bsc#1267045)
...
# Overlay vendored Go dependencies with CVE-fixed versions (bsc#1267045)
%{?_with_golang:rm -rf src/go/vendor && tar xzf %{SOURCE17} -C src/go/}
...
%{?_with_golang:install -Dm 0644 %{SOURCE18} %{buildroot}%{_unitdir}/zabbix_agent2.service}
...
%{?_with_golang:%pre agent2
%service_add_pre zabbix_agent2.service
}
...
%{?_with_golang:%post agent2
%service_add_post zabbix_agent2.service
}
...
%doc README-SSL.SUSE
%dir %{_sysconfdir}/zabbix
%config(noreplace) %attr(0640, root, %{agent_group}) %{_sysconfdir}/zabbix/zabbix_agentd.conf
...
%doc README-SSL.SUSE
%config(noreplace) %attr(0640, root, %{agent_group}) %{_sysconfdir}/zabbix/zabbix_agent2.conf
...
%{_unitdir}/zabbix_agent2.service
%attr(0770,root,%{agent_group}) %dir %{_localstatedir}/log/%{agent_user}
%ghost %attr(0770,root,%{agent_group}) %dir %{_rundir}/%{agent_user}
...
%{_prefix}/lib/zabbix-java-gateway/dnsjava-3.6.4.jar

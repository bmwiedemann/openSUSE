...
Name:           istio-proxy
Version:        1.1.0snapshot.2+git20181106
Release:        0
Summary:        L7 proxy and communication bus for Istio
License:        Apache-2.0
...
Source8:        %{name}-%{version}.tar.xz
Source9:        %{name}-BUILD
Source10:       %{name}-WORKSPACE
...
%description
Istio Proxy is an L7 proxy for microservices which forms a microservice mesh.
It's a part of Istio infrastructure.
...
cp %{SOURCE9} ./BUILD
cp %{SOURCE10} ./WORKSPACE
cp %{SOURCE11} ./repositories.bzl
...
%install
install -D -m0755 bazel-bin/src/envoy/envoy %{buildroot}%{_bindir}/istio-proxy

...
Requires:       python-requests >= 2.30.0
Requires:       python-scikit-learn >= 1.4.0
Requires:       python-scipy >= 1.9.0
...
BuildRequires:  %{python_module scipy >= 1.9.0}
# /SECTION
%python_subpackages
...
%pytest %{?with_pytest_xdist:-n auto} -s -m "not slow and not flaky and not single_process" -k "not ($donttest)" $ignorefiles && rm -r nilearn_cache
fi

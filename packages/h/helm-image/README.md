...
```ShellSession
$ podman run --rm -it  registry.opensuse.org/opensuse/helm:3.18 <helm-sub-command>
```

For instance, to display the Helm version, run:
```ShellSession
$ podman run --rm -it registry.opensuse.org/opensuse/helm:3.18 version --template='{{.Version}}'
v3.18
```
...
```ShellSession
$ podman run --rm -it --net=host -v /path/to/kubeconfig:/root/.kube/config:Z registry.opensuse.org/opensuse/helm:3.18
```

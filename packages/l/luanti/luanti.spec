...
%define luantiuser %{name}
%define luantigroup %{name}
...
Source:         https://github.com/luanti-org/%{name}/archive/%{version}/%{name}-%{version}.tar.gz
Source1:        luanti-rpmlintrc
Source2:        luanti@.service
BuildRequires:  cmake
...
Provides:       minetest = %{version}
Provides:       minetest-runtime = %{version}
Obsoletes:      minetest-lang < %{version}
...
Obsoletes:      minetestserver < %{version}
Provides:       minetest-runtime = %{version}
Provides:       minetestserver = %{version}
%{?systemd_requires}
...
Obsoletes:      minetest-data < %{version}
Provides:       minetest-data = %{version}
BuildArch:      noarch
...
getent passwd %{name} > /dev/null || \
  %{_sbindir}/useradd -r -g %{luantigroup} -d %{_localstatedir}/lib/%{name} \
  -s /sbin/nologin -c "user for %{name}server" %{luantiuser}
...
%attr(0755,%{luantiuser},%{luantigroup}) %{_localstatedir}/lib/%{name}/
...
%pretrans data
if [ -L "/usr/share/luanti/client/shaders/Irrlicht" ]; then
rm /usr/share/luanti/client/shaders/Irrlicht
fi

...
%endif
%global binfiles \
    %{llvm_ua_anchor} %{llvm_tools} %{llvm_ir_tools} %{llvm_elf_dwarf_tools} \
...
Source0:        https://github.com/llvm/llvm-project/releases/download/llvmorg-%{_version}/llvm-project-%{_version}.src.tar.xz
Source1:        https://github.com/llvm/llvm-project/releases/download/llvmorg-%{_version}/llvm-project-%{_version}.src.tar.xz.sig
Source2:        https://releases.llvm.org/release-keys.asc#/%{name}.keyring
...
# PATCH-FIX-UPSTREAM: Forward lib dir suffix for compiler-rt build.
Patch12:        compiler-rt-lib-suffix.patch
...
%prep
%setup -q -b 50 -b 51 -n llvm-project-%{_version}.src

# Remove subprojects that we don't need.
rm -r cross-project-tests flang flang-rt libclc libsycl llvm-libgcc mlir

# TODO: partially clean libc and libunwind?
...
rm clang/unittests/Driver/DistroTest.cpp

# We hardcode i586
rm clang/test/Driver/x86_features.c
...
# libcxx
%patch -P 15 -p1
rm libcxx/test/std/localization/locale.categories/category.time/locale.time.get.byname/get_monthname.pass.cpp
rm libcxx/test/std/localization/locale.categories/category.time/locale.time.get.byname/get_monthname_wide.pass.cpp
...
%cmake \
    -DLLVM_ENABLE_PROJECTS="clang%{?with_use_lld:;lld}" \
...
%cmake \
    -DLLVM_ENABLE_PROJECTS="clang;clang-tools-extra%{?with_bolt:;bolt}%{?with_lld:;lld}%{?with_lldb:;lldb}%{?with_polly:;polly}" \
    -DLLVM_ENABLE_RUNTIMES="compiler-rt%{?with_libcxx:;libcxx;libcxxabi}%{?with_openmp:;openmp}" \
...
%endif
    ...
    -DLIBOMPTEST_BUILD_STANDALONE:BOOL=ON \
...
#
# tar xf llvm-project-%{_version}.src.tar.xz
# pushd llvm-project-%{_version}.src/
# mkdir build; cd build
# cmake -G Ninja -DCMAKE_BUILD_TYPE=Release -DLLVM_ENABLE_SPHINX:BOOL=ON \
#     -DLLVM_BUILD_DOCS:BOOL=ON -DLLVM_ENABLE_PROJECTS=clang -DSPHINX_WARNINGS_AS_ERRORS:BOOL=OFF \
#     -DLLVM_INCLUDE_TESTS:BOOL=OFF -DLLVM_INCLUDE_BENCHMARKS:BOOL=OFF ../llvm
# ninja docs-{llvm,clang}-{html,man}
# popd
# tar --sort=name --owner=0 --group=0 --mtime="@${SOURCE_DATE_EPOCH}" \
#     --pax-option=exthdr.name=%d/PaxHeaders/%f,delete=atime,delete=ctime \
#     -cJf llvm-docs-%{_version}.src.tar.xz llvm-project-%{_version}.src/build/docs/{man,html}
# tar --sort=name --owner=0 --group=0 --mtime="@${SOURCE_DATE_EPOCH}" \
#     --pax-option=exthdr.name=%d/PaxHeaders/%f,delete=atime,delete=ctime \
#     -cJf clang-docs-%{_version}.src.tar.xz llvm-project-%{_version}.src/build/tools/clang/docs/{man,html}
...
for i in ftdetect ftplugin indent syntax; do
    cp -r llvm/utils/vim/$i %{buildroot}%{_datadir}/vim/site/
done
mv llvm/utils/vim/README llvm/utils/vim/README.vim
...
%if %{with lldb_python}
%if %{suse_version} >= 1600
%global swig_soabi abi3
%else
%global swig_soabi %(%{python_bin} -c "import sysconfig; print(sysconfig.get_config_var('SOABI'))")
%endif

# Python: fix binary libraries location.
rm %{buildroot}%{python_pkg_sitearch}/lldb/native/_lldb.%{swig_soabi}.so
liblldb=$(readlink %{buildroot}%{_libdir}/liblldb.so)
ln -vsf "../../../../${liblldb}" %{buildroot}%{python_pkg_sitearch}/lldb/native/_lldb.%{swig_soabi}.so
%endif
...
rm -r %{buildroot}%{_includedir}/clang-tidy/[a-z]*
rm -r %{buildroot}%{_includedir}/lld
rm -r %{buildroot}%{_libdir}/cmake/lld/
...
rm -r ../llvm/test/tools/llvm-exegesis
# We don't build examples.
rm -r ../llvm/test/Examples/{Kaleidoscope,OrcV2Examples}
# On armv6l, fpext frem(12.0f, 5.0f) to double = inf for some reason. On ppc relocation errors.
sed -i '1i; XFAIL: target=armv6{{.*}}, target=powerpc-{{.*}}' ../llvm/test/ExecutionEngine/MCJIT/frem.ll
# Disable tests that seem to hang (armv6) or fail with relocation errors (ppc).
sed -i '1i; UNSUPPORTED: target=armv6{{.*}}' ../llvm/test/{CodeGen/Generic/PBQP.ll,ExecutionEngine/Orc/global-ctor-with-cast.ll}
sed -i '1i; UNSUPPORTED: target=armv6{{.*}}\n; XFAIL: target=powerpc-{{.*}}' \
  ../llvm/test/ExecutionEngine/MCJIT/{mov64zext32,test-interp-vec-{arithm_{float,int},logical,setcond-{fp,int}}}.ll
# Sporadic crashes, need to investigate.
sed -i '1iUNSUPPORTED: target=armv6{{.*}}, target=armv7{{.*}}, target=i586-{{.*}}' ../llvm/test/Object/macho-invalid.test
...
sed -i 's/REQUIRES: webassembly-registered-target/REQUIRES: webassembly-registered-target,x86-registered-target/' \
  ../llvm/test/tools/llvm-debuginfo-analyzer/WebAssembly/{01-wasm-compare-logical-elements,01-wasm-select-logical-elements,03-wasm-incorrect-lexical-scope-typedef,04-wasm-missing-nested-enumerators,05-wasm-incorrect-lexical-scope-variable}.test
%ifarch ppc64le
# Sporadic failures, possibly races?
rm ../llvm/test/tools/llvm-cov/{multithreaded-report,sources-specified}.test
%endif
...
sed -i '1i// XFAIL: target=powerpc-{{.*}}' ../clang/test/Interpreter/{code-undo,execute{,-stmts,-weak},fail,global-dtor,lambda,simple-exception}.cpp
# Tests hang on armv6l.
sed -i '1i// UNSUPPORTED: target=armv6{{.*}}' \
  ../clang/test/{Interpreter/{code-undo,execute,execute-weak}.cpp,Modules/{preprocess-{build-diamond.m,decluse.cpp,module.cpp},string_names.cpp}}
# Tests use env LD_LIBRARY_PATH=[...]:$LD_LIBRARY_PATH, which doesn't seem to work as intended.
sed -i '1i// XFAIL: *' ../clang/test/Interpreter/{cxx20-modules.cppm,dynamic-library.cpp}
%ifarch ppc64le
# Sporadic failures, possibly races?
rm -r ../clang/test/ClangScanDeps
%endif
...
sed -i '1i// XFAIL: target=i586-{{.*}}' ../clang-tools-extra/test/clang-tidy/checkers/{abseil/duration-factory-scale.cpp,altera/struct-pack-align.cpp}
...
sed -i '1i# XFAIL: *' ../libcxx/test/libcxx/selftest/dsl/dsl.sh.py
# Several tests seem to hang on armv6l.
sed -i '1i// UNSUPPORTED: target=armv6{{.*}}' \
  ../libcxx/test/libcxx/utilities/memory/util.smartptr/race_condition.pass.cpp \
  ../libcxx/test/std/utilities/memory/util.smartptr/util.smartptr.{enab/enable_shared_from_this,shared/util.smartptr.shared.const/{deduction,weak_ptr},weak/util.smartptr.weak.{const/shared_ptr_deduction,mod/swap,obs/lock,spec/swap}}.pass.cpp
# No support for std::strong_order on long double for armv{6,7}.
sed -i '1i// XFAIL: target=armv{{(6|7).*}}' ../libcxx/test/std/language.support/cmp/cmp.alg/strong_order_long_double.verify.cpp
%{python_bin} bin/llvm-lit -sv --param enable_experimental=False runtimes/runtimes-bins/libcxx/test/

# There are undefined references to __cxa_* functions and "typeinfo for int".
sed -i '1i@ XFAIL: target=arm{{.*}}' ../libcxxabi/test/native/arm-linux-eabi/ttype-encoding-{0,9}0.pass.sh.s
%{python_bin} bin/llvm-lit -sv runtimes/runtimes-bins/libcxxabi/test/
%endif
%endif
popd
...
%{ua_install %llvm_ua_anchor} \
    ...
    %{lapply -p ua_bin_slave %llvm_ir_tools} \
...
%files
%license llvm/CREDITS.TXT llvm/LICENSE.TXT
%{lapply -p bin_path %llvm_ua_anchor %llvm_tools}
%{lapply -p bin_path %llvm_ua_anchor %llvm_ir_tools}
...
%{lapply -p bin_sonum_path %llvm_ua_anchor %llvm_ir_tools}
...
%{lapply -p ghost_ua_bin_link %llvm_ua_anchor %llvm_ir_tools}
...
%endif
%ifarch aarch64 %{arm} loongarch64 ppc64le s390x x86_64
%{_libdir}/clang/%{_sonum}/lib/linux/liborc_rt-*.a
%endif

%files -n libclang_rt%{_sonum}
%license compiler-rt/CREDITS.TXT compiler-rt/LICENSE.TXT
%dir %{_libdir}/clang
%dir %{_libdir}/clang/%{_sonum}
...
%license libcxx/CREDITS.TXT libcxx/LICENSE.TXT
%{_libdir}/libc++.so.*

%files %{multisource libcxxabi%{_socxx}} libc++abi%{_socxx}
%license libcxxabi/CREDITS.TXT libcxxabi/LICENSE.TXT
%{_libdir}/libc++abi.so.*

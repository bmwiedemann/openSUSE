...
```Dockerfile
FROM registry.opensuse.org/opensuse/bci/gcc:16
WORKDIR /src/
COPY . /src/
...
```Dockerfile
FROM registry.opensuse.org/opensuse/bci/gcc:16 as builder
WORKDIR /src/
COPY . /src/
...
repository:
- `gcc16-ada` for the Ada frontend (GNAT)
- `gcc16-go` for the Go frontend
- `gcc16-objc` and `gcc16-obj-c++` for the Objective C and Objective C++
- `gcc16-d` for the frontend to the D Language
- `gcc16-m2` for the Modula 2 compiler frontend
- `gcc16-rust` for the GNU Rust compiler
...
```bash
podman run --rm -it -v $(pwd):/src/:Z registry.opensuse.org/opensuse/bci/gcc:16 \
    gcc -o /src/app.out /src/*.c
```
or by invoking `make`
```bash
podman run --rm -it -v $(pwd):/src/:Z --workdir /src/ \
    registry.opensuse.org/opensuse/bci/gcc:16 \
    make
```

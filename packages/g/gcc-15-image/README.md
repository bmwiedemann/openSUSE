...
```Dockerfile
FROM registry.opensuse.org/opensuse/bci/gcc:15
WORKDIR /src/
COPY . /src/
...
```Dockerfile
FROM registry.opensuse.org/opensuse/bci/gcc:15 as builder
WORKDIR /src/
COPY . /src/
...
repository:
- `gcc15-ada` for the Ada frontend (GNAT)
- `gcc15-go` for the Go frontend
- `gcc15-objc` and `gcc15-obj-c++` for the Objective C and Objective C++
- `gcc15-d` for the frontend to the D Language
- `gcc15-m2` for the Modula 2 compiler frontend
- `gcc15-rust` for the GNU Rust compiler
...
```bash
podman run --rm -it -v $(pwd):/src/:Z registry.opensuse.org/opensuse/bci/gcc:15 \
    gcc -o /src/app.out /src/*.c
```
or by invoking `make`
```bash
podman run --rm -it -v $(pwd):/src/:Z --workdir /src/ \
    registry.opensuse.org/opensuse/bci/gcc:15 \
    make
```

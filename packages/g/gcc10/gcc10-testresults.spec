...
%define itsme10 1
%define plv_ %{!?product_libs_gcc_ver:10}%{?product_libs_gcc_ver}
%define plv() %{expand:%%{!?itsme%{expand:%%{!?product_libs_gcc_ver_%{1}%{2}:%%{plv_}}%%{?product_libs_gcc_ver_%{1}%{2}}}:-gcc10}}
...
%endif
%if %{build_ada}
%if 0%{?gcc_version:%{gcc_version}} > 10
%define hostsuffix %{binsuffix}
BuildRequires:  gcc10-ada
...
%if !0%{?building_testsuite:1}
Requires:       binutils
Requires:       cpp10 = %{version}-%{release}
...
%endif
Suggests:       gcc10-info gcc10-locale
%endif

...
%define pkgname cross-riscv64-elf-gcc15-bootstrap
...
Source1:        change_spec
Source2:        gcc15-rpmlintrc
Source3:        gcc15-testresults-rpmlintrc
Source4:        README.First-for.SuSE.packagers
Source5:        newlib-4.5.0.20241231.tar.xz
...
%package -n cross-%cross_arch-gcc15-icecream-backend
Summary:        Icecream backend for the GNU C Compiler
Group:          Development/Languages/C and C++

%description -n cross-%cross_arch-gcc15-icecream-backend
This package contains the icecream environment for the GNU C Compiler
%endif

%if 0%{?nvptx_newlib:1}
%package -n cross-nvptx-newlib15-devel
...
%description -n cross-amdgcn-newlib15-devel
...
%{_sbindir}/update-alternatives \
  --install %{_bindir}/%{gcc_target_arch}-gcc %{gcc_target_arch}-gcc %{_bindir}/%{gcc_target_arch}-gcc%{binsuffix} 15 \
...
%if 0%{?gcc_icecream:%gcc_icecream}
%files -n cross-%cross_arch-gcc15-icecream-backend
%defattr(-,root,root)
/usr/share/icecream-envs
%endif

%if 0%{?nvptx_newlib:1}
%files -n cross-nvptx-newlib15-devel
%defattr(-,root,root)
%{_prefix}/%{gcc_target_arch}
%endif

%if 0%{?amdgcn_newlib:1}
%files -n cross-amdgcn-newlib15-devel
%defattr(-,root,root)
%{_prefix}/%{gcc_target_arch}

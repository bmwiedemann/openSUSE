...
%define original_name godot
%define original_name_upper Godot
%define branch 3
%define name_upper %{original_name_upper}%{branch}
...
exporting Godot games on the command line. %{version_text}
...
by pointing to the location of the game's data package. %{version_text}
...
particularly suited for running dedicated servers. %{version_text}
...
sed -i 's/\/usr\/share\/doc\/godot\//\/usr\/share\/doc\/packages\/godot3\//' misc/dist/linux/godot.6
...
cp misc/dist/linux/org.godotengine.Godot.desktop misc/dist/linux/org.godotengine.%{name_upper}.desktop

# disarm shebang
sed -i '1s/#!/##/' misc/dist/shell/godot.bash-completion

# bash completion for sub packages
cp misc/dist/shell/%{original_name}.bash-completion misc/dist/shell/%{name}-headless
cp misc/dist/shell/%{original_name}.bash-completion misc/dist/shell/%{name}-runner
cp misc/dist/shell/%{original_name}.bash-completion misc/dist/shell/%{name}-server

sed -i '$s/_complete_godot_bash %{original_name}/_complete_godot_bash %{name}/' misc/dist/shell/%{original_name}.bash-completion
...
suffix=%{__isa_bits}
%endif
install -D -p -m 755 bin/%{original_name}.x11.opt.tools.$suffix %{buildroot}%{_bindir}/%{name}

install -D -p -m 644 misc/dist/linux/%{original_name}.6 %{buildroot}/%{_mandir}/man6/%{name}.6%{?ext_man}
...
install -D -p -m 755 bin/%{original_name}_server.x11.opt.tools.$suffix %{buildroot}%{_bindir}/%{name}-headless

# Installing the runner
install -D -p -m 755 bin/%{original_name}.x11.opt.$suffix %{buildroot}%{_bindir}/%{name}-runner

# Installing the server
install -D -p -m 755 bin/%{original_name}_server.x11.opt.$suffix %{buildroot}%{_bindir}/%{name}-server
%endif

# Installing bash-completion
install -D -p -m 644 misc/dist/shell/%{original_name}.bash-completion %{buildroot}%{_datadir}/bash-completion/completions/%{name}
install -D -p -m 644 misc/dist/shell/%{name}-headless %{buildroot}%{_datadir}/bash-completion/completions/%{name}-headless
install -D -p -m 644 misc/dist/shell/%{name}-runner %{buildroot}%{_datadir}/bash-completion/completions/%{name}-runner
...
%{_bindir}/%{name}
%{_datadir}/metainfo/org.godotengine.%{name_upper}.appdata.xml
%{_datadir}/applications/org.godotengine.%{name_upper}.desktop
